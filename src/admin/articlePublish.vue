<template>
  <div id="articlePublish">
    <div class="container">
      ...
      <el-input
        v-if="isShow"
        v-model="publishDate.label1"
        style="width: 200px"
        placeholder="请输入内容"
      ></el-input>
      <el-input
        v-if="isShow"
        v-model="publishDate.label2"
        style="width: 200px"
        placeholder="请输入内容"
      ></el-input>
      <el-input
        v-if="isShow"
        v-model="publishDate.label3"
        style="width: 200px"
        placeholder="请输入内容"
      ></el-input>
      <el-input
      ...
      <div class="event-Button">
        <el-button @click="handeleClick" type="primary">{{
          btntext
        }}</el-button>
        <el-button v-show="!isShow" @click="handleCancel" type="default"
          >取消</el-button
        >
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async getArticleInfo(id) {
      const res = await this.$http.get("/api/article/detail", {
    ...
    async handleUpdate() {
      ...
      if (
        ...
        !this.publishDate.label1 ||
        !this.publishDate.label2 ||
        !this.publishDate.label3
      )
    ...
    console.log(Cookie.get("token"))
      let res = await this.$http.post('/article/add', {
          title:this.publishDate.title,
          content:this.publishDate.content,
          classname01:this.publishDate.label1,
          classname02:this.publishDate.label2,
          classname03:this.publishDate.label3,
          type:1,
          pic_url:""
      })
      if (res.code == 200) {
